...
class EvaluationService {
    ...
    getAllEvaluations(filters) {
        try {
            ...
            evaluations.forEach(evaluation => {
                const result = caseCountStmt.get(evaluation.id);
                evaluation.case_count = result.case_count;
            });
            return evaluations;
        }
        catch (error) {
            logger_1.default.error('Error getting evaluations:', error);
    ...
    getEvaluationById(id) {
        try {
            ...
            logger_1.default.error('Error getting evaluation by ID:', error);
    ...
    createEvaluation(evaluationData, createdBy) {
        try {
            ...
            const coachStmt = db.prepare(`
                ...
                LIMIT 1
            `);
            const assignment = coachStmt.get(evaluationData.engineer_id);
            if (!assignment) {
            ...
            const stmt = db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?)
            `);
            const result = stmt.run(evaluationData.engineer_id, assignment.coach_user_id, evaluationData.evaluation_date, createdBy, createdBy);
            // Create 7 default case evaluations
            this.createDefaultCases(result.lastInsertRowid);
            const newEvaluation = this.getEvaluationById(result.lastInsertRowid);
            if (!newEvaluation) {
                throw new Error('Failed to retrieve created evaluation');
            }
            logger_1.default.info(`Evaluation created: Engineer ${newEvaluation.engineer_name} for ${evaluationData.evaluation_date}`);
            return newEvaluation;
        }
        catch (error) {
            logger_1.default.error('Error creating evaluation:', error);
            throw new Error(error instanceof Error ? error.message : 'Failed to create evaluation');
        }
    }
    // Update evaluation
    updateEvaluation(id, updateData, updatedBy) {
        try {
            ...
            logger_1.default.info(`Evaluation updated: ID ${id} by user ${updatedBy}`);
            return updatedEvaluation;
        }
        catch (error) {
            logger_1.default.error('Error updating evaluation:', error);
            throw new Error('Failed to update evaluation');
        }
    }
    // Soft delete evaluation
    deleteEvaluation(id, deletedBy) {
        try {
            const existingEvaluation = this.getEvaluationById(id);
            if (!existingEvaluation) {
                throw new Error('Evaluation not found');
            ...
            logger_1.default.info(`Evaluation soft deleted: ID ${id} (Engineer: ${existingEvaluation.engineer_name}, Date: ${existingEvaluation.evaluation_date}) by user ${deletedBy}`);
        }
        catch (error) {
            logger_1.default.error('Error deleting evaluation:', error);
            throw new Error('Failed to delete evaluation');
        }
    }
    // Get evaluations by coach
    getEvaluationsByCoach(coachUserId) {
        try {
            return this.getAllEvaluations({ coachUserId });
        }
        catch (error) {
            logger_1.default.error('Error getting evaluations by coach:', error);
    ...
    getEvaluationsByLead(leadUserId) {
        try {
            ...
            logger_1.default.error('Error getting evaluations by lead:', error);
    ...
    getCasesByEvaluationId(evaluationId) {
        try {
            ...
            logger_1.default.error('Error getting cases by evaluation ID:', error);
    ...
    createDefaultCases(evaluationId) {
        try {
            ...
            logger_1.default.error('Error creating default cases:', error);
            throw new Error('Failed to create default cases');
        }
    }
    // Add new case to evaluation
    addCase(evaluationId, caseData) {
        try {
            ...
            `);
            const insertResult = stmt.run(evaluationId, nextCaseNumber, caseData.case_id || null, caseData.kb_potential ? 1 : 0, caseData.article_linked ? 1 : 0, caseData.article_improved ? 1 : 0, caseData.improvement_opportunity ? 1 : 0, caseData.article_created ? 1 : 0, caseData.create_opportunity ? 1 : 0, caseData.relevant_link ? 1 : 0, caseData.notes || null);
            const newCase = this.getCaseById(insertResult.lastInsertRowid);
            if (!newCase) {
                throw new Error('Failed to retrieve created case');
            }
            logger_1.default.info(`Case added to evaluation ${evaluationId}: Case #${nextCaseNumber} (ID: ${caseData.case_id || 'none'})`);
            return newCase;
        }
        catch (error) {
            logger_1.default.error('Error adding case:', error);
    ...
    deleteCase(caseId) {
        try {
            ...
            `);
            stmt.run(caseId);
            logger_1.default.info(`Case soft deleted: ID ${caseId}`);
        }
        catch (error) {
    ...
    checkCaseIdExists(caseId) {
        try {
            ...
            const stmt = db.prepare(`
                ...
                LIMIT 1
            `);
            const result = stmt.get(caseId.trim());
            if (result) {
                return {
                    exists: true,
                    evaluation: {
            ...
            logger_1.default.error('Error checking case ID existence:', error);
            throw new Error('Failed to check case ID');
        }
    }
    // Generate statistics for reports
    generateStats(filters) {
        try {
            let query = `
                SELECT 
            ...
            logger_1.default.error('Error generating stats:', error);
            throw new Error('Failed to generate statistics');
        }
    }
}
exports.default = new EvaluationService();
